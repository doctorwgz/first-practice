...
const unlockedLabel = computed(() => {
  ...
  const tierTitle = myTier.value.title
  return unlocked.value.map(u => {
    const name = u.name || ''
    if (name.includes('挂件')) {
      return name.replace(/挂件\d?/, tierTitle + '挂件')
    }
    return name
  }).join('+')
})
  ...
  <div class="screen lb no-scrollbar">
    ...
    <div class="lb-scroll no-scrollbar" :class="{ 'guest-pad': isGuest }" :style="bgStyle('/assets/activity/bg.png')">
      ...
      <div class="hero-spacer">
        <div class="countdown">
          <span class="cd-label">距活动结束</span>
          <span class="cd-box">{{ remain.d }}</span><i>天</i>
          <span class="cd-box">{{ pad(remain.h) }}</span><i>:</i>
          <span class="cd-box">{{ pad(remain.m) }}</span><i>:</i>
          <span class="cd-box">{{ pad(remain.s) }}</span>
        </div>
      </div>
        ...
        <div class="train-inner">
          ...
          <div class="train-row owner">
            <div class="owner-info">
              <div class="owner-name">{{ myRank?.name }}</div>
              <div class="owner-id">传奇榜得主 (ID: {{ myRank?.id }})</div>
            </div>
      ...
      <section class="card ranklist" :style="bgStyle('/assets/activity/bg-rank-list.png')">
        <div class="rank-inner">
          <div class="joined">{{ joinedCount }} 人参战 · 消耗即参战</div>
      ...
      <section class="card desc" :style="bgStyle('/assets/activity/bg-desc.png')">
        <div class="desc-inner">
          <p>
             "AI时代下的精英成长计划"活动期间，每人赠送1000 Token训练包（4.11-4.24有效），消耗越多等级越高：≥200 Token得"探索者"挂件+《机构猎手AI选股》电子书，≥500 Token得"领航者"挂件+200Token返场卡，≥800 Token得"达人"挂件+400Token返场卡；每使用1次token进行训练，都讲实时计入 TOKEN传奇榜，排名全平台可见。现在就开练，边拿权益边冲榜！
          </p>
        </div>
      </section>
    </div>
