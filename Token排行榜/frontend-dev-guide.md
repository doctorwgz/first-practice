...
**说明：** 后端一次性返回前100名，前端自己控制默认显示8条 vs 展开全部。
...
| 精网号 | 昵称 | 类型 | 密码 | 消耗 | 等级 |
|--------|------|------|------|------|------|
| 90123456 | 会员张三 | 会员(1) | `123456` | 520 | 领航者 |
| 90123457 | 会员李四 | 会员(1) | `123456` | 950 | 达人 |
| 94123456 | 非网王五 | 非网(2) | `123456` | 150 | 无 |
| 99123456 | 游客赵六 | 游客(3) | `123456` | 0 | 无 |
...
后端已返回 `next_level_need`（距下一级还需多少 Token）和 `next_level_name`（下一级称号），前端直接拼文案：

| 场景 | 显示文案 |
|--------|----------|
| `next_level_need > 0` | `再使用 ${next_level_need} Token 即可成为${next_level_name}` |
| `next_level_need === 0` | `恭喜已达最高等级！继续训练冲击传奇榜榜首` |
