...
/* ---------- 深度探索进度指示 ---------- */
const tplRowRef = ref(null)
const tplDotIndex = ref(0)

function onTplScroll () {
  const el = tplRowRef.value
  if (!el) return
  ...
  tplDotIndex.value = el.scrollLeft / maxScroll > 0.5 ? 1 : 0
}
...
/* ---------- Token 消耗提示 ---------- */
const consumeTip = ref({ show: false, text: '', type: 'success' })
let consumeTipTimer = null

function showTip(text, type = 'success') {
  consumeTip.value = { show: true, text, type }
  clearTimeout(consumeTipTimer)
  consumeTipTimer = setTimeout(() => { consumeTip.value.show = false }, 2500)
}
...
async function onSearch() {
  try {
    const data = await request.post('/token/consume', { amount: 20, scene: 'AI搜索' })
    console.log('消耗成功', data)
    showTip('本次已消耗20Token', 'success')
  } catch (e) {
    console.error('消耗Token失败', e)
    // request.js 已将 GoFrame 的 code≠0 包装成 Error(message)
    showTip(e.message || '请求失败', 'error')
  }
  keyword.value = ''
}
...
async function loadHomeData() {
  ...
  { name: '富时新加坡海峡时报', chg: 0.07 }     // 10 右
    ...
    if (tplRes?.items?.length) {
      exploreTemplates.value = tplRes.items.map(t => ({
        name: t.name,
        icon: t.icon
      }))
    }
...
onMounted(() => {
  ...
  if (savedScroll && scrollEl.value) {
    requestAnimationFrame(() => {
      scrollEl.value.scrollTop = parseInt(savedScroll)
    })
  }
})
...
onBeforeUnmount(() => {
  ...
  // 保存滚动位置（返回首页时恢复）
  if (scrollEl.value) {
    sessionStorage.setItem('home_scroll_top', scrollEl.value.scrollTop)
  }
})
...
<template>
  <div class="screen home no-scrollbar">
    <!-- Token 消耗提示 -->
    <transition name="tip-down">
      <div
        v-if="consumeTip.show"
        class="consume-tip"
        :class="consumeTip.type"
      >
        <span>{{ consumeTip.text }}</span>
      </div>
    </transition>

    <!-- 滚动内容区(包含顶部机器人) -->
    <main ref="scrollEl" class="home-scroll no-scrollbar">
      <!-- 顶部问候头(珊瑚渐变,向下淡出为白) -->
      <header class="hero">
        <StatusBar />
        <div class="greet-row">
  ...
  <div class="roll">
    <transition name="roll-up" mode="out-in">
      ...
      <section class="sec explore">
        ...
        <div class="tpl-wrap">
          <div ref="tplRowRef" class="tpl-row no-scrollbar" @scroll="onTplScroll">
            <button
              ...
              @click="goPlaceholder"
            >
              <img class="tpl-ic" :src="t.icon" alt="" />
              <span class="tpl-name">{{ t.name }}</span>
            </button>
          </div>
          <div class="tpl-dots">
            <i v-for="i in 2" :key="i" :class="{ on: i - 1 === tplDotIndex }"></i>
          </div>
        </div>
      </section>
    ...
    <div class="home-bottom">
      <div class="search-row">
        <img :src="asset('/assets/home/deepmate_logo.png')" class="search-ai-outside" alt="" />
...
.wl-card {
  ...
  border-radius: 14px;
  padding: 12px 10px 0;
  box-shadow: 0 6px 16px rgba(26, 26, 27, 0.05);
  overflow: hidden;
}
.wl-name {
  font-size: 12px;
...
.combined-card .wl-row {
  margin-top: 10px;
  margin-bottom: 14px;
}
.combined-card .briefs {
...
.search-bar {
  ...
  background: linear-gradient(90deg, #f57e7e, #f8bfc3);
  border-radius: 22px;
...
/* 提示条下滑动画 */
.tip-down-enter-active,
.tip-down-leave-active {
